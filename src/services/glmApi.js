  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorMessage = errorData?.error?.message || `HTTP错误! 状态码: ${response.status}`
      } catch {
        errorMessage = `HTTP错误! 状态码: ${response.status} - ${errorText}`
      }
    ...
    try {
      ...
      console.log('JSON解析成功，生成了', Array.isArray(parsed) ? parsed.length : 0, '个方案')
      return parsed
    } catch (parseError) {
